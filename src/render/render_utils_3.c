...
void	draw_walls_utils(t_sdl *sdl, SDL_Surface *s)
{
	...
	sdl->render.draw_end = sdl->render.line_height / 2 + (SCREEN_HEIGHT -
	((sdl->render.line_height - 1) * 2 * sdl->floor) + sdl->up_down) / 2;
...
void	draw_walls(t_sdl *sdl)
{
	...
	sdl->render.line_height = (int)(SCREEN_HEIGHT / sdl->render.perp_wall_dist);
	sdl->render.draw_start = -sdl->render.line_height / 2 + (SCREEN_HEIGHT -
	((sdl->render.line_height - 1) * 2 * sdl->floor) + sdl->up_down) / 2;
