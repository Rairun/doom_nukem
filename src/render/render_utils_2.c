...
void		draw_ceiling_floor_utils(t_sdl *sdl)
{
	...
	while (sdl->render.draw_end <= SCREEN_HEIGHT + abs(sdl->up_down))
	{
		sdl->render.current_dist = (SCREEN_HEIGHT + sdl->up_down) / (2.0 *
		sdl->render.draw_end - (SCREEN_HEIGHT + sdl->up_down));
		sdl->render.weight = fabs(sdl->render.current_dist /
		(sdl->render.dist_wall + (sdl->render.dist_wall * sdl->look)));
